...
def clr(text: str, color_code: str) -> str:
    """Wraps text in ANSI escape codes if the environment supports it."""
...
for plugin in plugins_config:
    ...
    if result_configure.returncode != 0:
        ...
        build_results.append({"plugin": name, "target": "CMake Config", "status": "Failed"})
        continue

    if args.configure_only:
        build_results.append({"plugin": name, "target": "CMake Config", "status": "Success"})
    else:
        for fmt in formats:
...
if build_results:
    ...
    for r in build_results:
        plugin = r["plugin"].ljust(max_plugin_len)
        target = r["target"].ljust(max_target_len)
        status_raw = r["status"]
        if status_raw == "Success":
            status = clr(status_raw, GREEN)
        else:
            status = clr(status_raw, RED)
