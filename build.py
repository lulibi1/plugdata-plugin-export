...
def validate_config(path: str) -> list:
    ...
    if not os.path.isfile(path):
        print(clr(f"FATAL: config.json not found at '{os.path.abspath(path)}'", BOLD, RED))
        sys.exit(1)
    ...
    try:
        ...
        print(clr(f"FATAL: config.json is not valid JSON – {e}", BOLD, RED))
        sys.exit(1)

    if not isinstance(data, list):
        print(clr("FATAL: config.json must contain a JSON array of plugin objects.", BOLD, RED))
        sys.exit(1)
...
if warnings:
    print(clr("Build warnings:", BOLD, YELLOW))
    for w in warnings:
        print(clr(w, YELLOW))
    print()

if errors:
    print(clr("Build errors – cannot continue:", BOLD, RED))
    for e in errors:
        print(clr(e, RED))
    sys.exit(1)
...
if not plugdata_dir.is_dir():
    print(clr(f"FATAL: plugdata directory not found at '{plugdata_dir}'. "
              ...
              f"the plugdata submodule has been initialised (git submodule update --init).", BOLD, RED))
    sys.exit(1)
...
for plugin in plugins_config:
    ...
    print(f"\n{clr('Processing:', CYAN)} {clr(name, BOLD)}")
    ...
    if result_configure.returncode != 0:
        print(clr(f"Failed cmake configure for {name}", BOLD, RED))
        continue
    ...
    if not args.configure_only:
        for fmt in formats:
            ...
            if result_build.returncode != 0:
                print(clr(f"Failed to build target: {target}", BOLD, RED))
            else:
