...
def clr(text: str, color_code: str) -> str:
    """Helper to wrap text in ANSI escape codes if outputting to a TTY."""
...
def validate_plugin(plugin: dict, index: int):
    ...
    if not patch:
        ...
        error(f"{prefix} ({name!r}): 'patch' must be a non-empty string.")
...
for i, plugin in enumerate(plugins_config):
    ...
    print(clr(f"\n[{i+1}/{total_plugins}] Processing: {name}", "34"))
    ...
    if not args.configure_only:
        for fmt in formats:
            ...
            print(clr(f"Building target: {target}", "34"))
...
print(clr(f"\nBuild Summary:", "34"))
print(clr(f"  Total plugins: {total_plugins}", "34"))
print(clr(f"  Successful:    {successful_builds}", "92"))
if failed_builds > 0:
    print(clr(f"  Failed:        {failed_builds}", "91"))
