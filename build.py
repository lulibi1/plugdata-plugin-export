...
if build_results:
    ...
    p_width = max(len("Plugin"), max(len(p) for p, t, s in build_results))
    t_width = max(len("Target"), max(len(t) for p, t, s in build_results))

    border_len = p_width + t_width + 12
    border = "-" * border_len

    print(border)
    header = clr(f"{'Plugin':<{p_width}}  {'Target':<{t_width}}  {'Status':<8}", 1)
    print(header)
    print(border)

    for plugin, target, status in build_results:
        if status == "SUCCESS":
            colored_status = clr("SUCCESS", 92)
        else:
            colored_status = clr("FAILED", 91)

        padding = " " * (8 - len(status))
        print(f"{plugin:<{p_width}}  {target:<{t_width}}  {colored_status}{padding}")
