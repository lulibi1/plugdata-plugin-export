...
build_results = [] # list of tuple: (plugin_name, target/phase, status)
...
if build_results:
    print(clr("\nBuild Summary", 1, 34))
    p_width = max(6, max(len(p) for p, t, s in build_results))
    t_width = max(6, max(len(t) for p, t, s in build_results))

    header = f"{'Plugin':<{p_width}}  {'Target':<{t_width}}  {'Status':<8}"
    print(header)
    print("-" * (p_width + t_width + 12))

    for p, t, s in build_results:
        status_color = 92 if s == "SUCCESS" else 91
        padded_status = clr(s, status_color)
        padding = " " * (8 - len(s))
        print(f"{p:<{p_width}}  {t:<{t_width}}  {padded_status}{padding}")
