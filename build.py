...
build_results = [] # track (plugin_name, target_name, status)
...
for plugin in plugins_config:
    ...
    if result_configure.returncode != 0:
        ...
        build_results.append((name, "CMake Configure", clr("FAILED", 91)))
        continue

    build_results.append((name, "CMake Configure", clr("SUCCESS", 92)))
    ...
    if not args.configure_only:
        for fmt in formats:
            ...
            if result_build.returncode != 0:
                ...
                build_results.append((name, target, clr("FAILED", 91)))
            else:
                print(clr(f"Successfully built: {target}", 92))
                build_results.append((name, target, clr("SUCCESS", 92)))
...
if build_results:
    ...
    p_header = clr("Plugin".ljust(p_width), 1)
    t_header = clr("Target".ljust(t_width), 1)
    s_header = clr("Status", 1)

    print(f"{p_header}   {t_header}   {s_header}")
