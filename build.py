...
def error(msg: str):
    errors.append(f"  {clr('ERROR:', RED, BOLD)} {msg}")

def warn(msg: str):
    warnings.append(f"  {clr('WARNING:', YELLOW, BOLD)} {msg}")

def validate_config(path: str) -> list:
    """Load and validate config.json. Returns the parsed list or exits."""
    if not os.path.isfile(path):
        print(f"{clr('FATAL:', RED, BOLD)} config.json not found at '{os.path.abspath(path)}'")
        sys.exit(1)
    ...
    try:
        ...
        print(f"{clr('FATAL:', RED, BOLD)} config.json is not valid JSON – {e}")
        sys.exit(1)

    if not isinstance(data, list):
        print(f"{clr('FATAL:', RED, BOLD)} config.json must contain a JSON array of plugin objects.")
        sys.exit(1)
...
if not plugdata_dir.is_dir():
    print(f"{clr('FATAL:', RED, BOLD)} plugdata directory not found at '{plugdata_dir}'. "
...
for plugin in plugins_config:
    ...
    print(f"\n{clr('Processing:', CYAN, BOLD)} {clr(name, BOLD)}")
    ...
    if result_configure.returncode != 0:
        print(f"{clr('Failed cmake configure for', RED, BOLD)} {name}")
        continue
    ...
    if not args.configure_only:
        for fmt in formats:
            ...
            if result_build.returncode != 0:
                print(f"{clr('Failed to build target:', RED, BOLD)} {target}")
            else:
                print(f"{clr('Successfully built:', GREEN, BOLD)} {target}")
