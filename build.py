...
def clr(t, c):
    if os.environ.get("NO_COLOR") or not (sys.stdout.isatty() or os.environ.get("FORCE_COLOR")): return t
...
def validate_plugin(plugin: dict, index: int):
    ...
    if not path:
        ...
        if not resolved.exists():
            ...
            error(f"{prefix} ({name!r}): plugin path must be a file or directory: '{resolved}'")
...
for plugin in plugins_config:
    name, formats = plugin["name"], plugin.get("formats", [])
    zip_path, patch = Path(plugin["path"]).resolve(), plugin["patch"]
    ...
    if plugin_success: stats["success"] += 1
    else: stats["fail"] += 1
