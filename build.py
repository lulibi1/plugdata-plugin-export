...
def clr(text: str, code: str) -> str:
    """Wrap text in ANSI color codes if stdout is a TTY and colors aren't disabled."""
...
for plugin in plugins_config:
    ...
    if result_configure.returncode != 0:
        ...
        build_results.append((name, "CMake Configure", False))
        continue
    else:
        build_results.append((name, "CMake Configure", True))
    ...
    if not args.configure_only:
        for fmt in formats:
            ...
            print(f"{clr('Building target:', '34')} {target}")
...
if build_results:
    ...
    max_name = max(len(r[0]) for r in build_results)
    max_target = max(len(r[1]) for r in build_results)

    for name, target, success in build_results:
        status_text = clr("SUCCESS", "92") if success else clr("FAILED", "91")
        print(f"{name:<{max_name}} | {target:<{max_target}} | {status_text}")
