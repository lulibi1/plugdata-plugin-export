...
for plugin in plugins_config:
    ...
    if not args.configure_only:
        ...
        for fmt in formats:
            ...
            print(f"{clr('Building target', BLUE)}: {target}")
...
print(f"Successful:    {clr(str(successful_builds), GREEN)}")
print(f"Failed:        {clr(str(failed_builds), RED if failed_builds > 0 else GREEN)}")
