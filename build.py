...
# ── End of Execution Build Summary ─────────────────────────────────────────

if build_results:
    print("\n" + clr("=== BUILD SUMMARY ===", "1", "34"))
    p_width = max(len("Plugin"), max((len(r[0]) for r in build_results), default=0))
    t_width = max(len("Target"), max((len(r[1]) for r in build_results), default=0))
    s_width = 10

    header = f"{'Plugin':<{p_width}}  {'Target':<{t_width}}  {'Status':<{s_width}}"
    sep_len = max(len(header), p_width + t_width + 20)
    print(header)
    print("-" * sep_len)
    ...
    for p_name, t_name, status in build_results:
        if status == "SUCCESS":
            status_str = clr("SUCCESS", "92", "1")
            success_count += 1
        else:
            status_str = clr("FAILED", "91", "1")
            fail_count += 1

        pad = " " * (s_width - len(status))
        print(f"{p_name:<{p_width}}  {t_name:<{t_width}}  {status_str}{pad}")

    print("-" * sep_len)
    total = len(build_results)
    if fail_count == 0:
        summary_msg = f"Build completed successfully! ({success_count}/{total} succeeded) -> Artifacts in '{build_output_dir}'"
        print(clr(summary_msg, "92", "1"))
    else:
        summary_msg = f"Build finished with errors: {success_count} succeeded, {fail_count} failed out of {total} targets."
        print(clr(summary_msg, "91", "1"))
