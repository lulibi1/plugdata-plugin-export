...
def clr(text: str, code: str) -> str:
    """Helper to colorize text with ANSI codes."""
...
def error(msg: str):
    errors.append(clr(f"  ERROR: {msg}", "1;91"))

def warn(msg: str):
    warnings.append(clr(f"  WARNING: {msg}", "1;93"))
    ...
    if not args.configure_only:
        for fmt in formats:
            ...
            if result_build.returncode != 0:
                print(clr(f"Failed to build target: {target}", "1;91"))
                build_results.append((name, fmt, "FAILED"))
            else:
                print(clr(f"Successfully built: {target}", "1;92"))
                build_results.append((name, fmt, "SUCCESS"))
...
if build_results:
    p_w = max(max(len(r[0]) for r in build_results), 6)
    t_w = max(max(len(r[1]) for r in build_results), 6)
    b = "=" * (p_w + t_w + 20)
    print("\n" + clr(b, "1;34"))
    print(clr("BUILD SUMMARY".center(p_w + t_w + 20), "1;34"))
    print(clr(b, "1;34"))
    print(f"| {'Plugin':<{p_w}} | {'Target':<{t_w}} | {'Status':<10} |")
    print("-" * (p_w + t_w + 20))
    for name, target, status in build_results:
        st_clr = clr(status, "1;92" if status == "SUCCESS" else "1;91")
        print(f"| {name:<{p_w}} | {target:<{t_w}} | {st_clr}{' ' * (10 - len(status))} |")
    print(b)
