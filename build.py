...
def clr(text: str, color_code: int) -> str:
    """Wraps text in ANSI color codes if stdout is a TTY or FORCE_COLOR is set."""
...
for i, plugin in enumerate(plugins_config):
    ...
    print(f"\n[{i+1}/{len(plugins_config)}] {clr('Processing', BLUE)}: {name}")
    ...
    if not args.configure_only:
        for fmt in formats:
            ...
            print(f"Building target: {clr(target, BLUE)}")
