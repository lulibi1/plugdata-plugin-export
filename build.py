...
def clr(text: str, *codes) -> str:
    """Applies ANSI terminal escape codes if colors are enabled."""
    if "NO_COLOR" in os.environ:
        return text
    if "FORCE_COLOR" in os.environ or (sys.stdout and sys.stdout.isatty()):
...
for plugin in plugins_config:
    ...
    if result_configure.returncode != 0:
        print(f"{clr('Failed cmake configure for', 1, 91)} {name}")
        continue
    ...
    if not args.configure_only:
        for fmt in formats:
            ...
            if result_build.returncode != 0:
                print(f"{clr('Failed to build target:', 1, 91)} {target}")
            else:
                print(f"{clr('Successfully built:', 1, 92)} {target}")
